...
- [only logs to Syslog (runs in chroot jail, no file or socket access)](#only-logs-to-syslog-runs-in-chroot-jail-no-file-or-socket-access)
- [default, use for SSL, SSH, SMTP, IMAP etc](#mode-tcp--default-use-for-ssl-ssh-smtp-imap-etc)
- [dissects HTTP contents](#dissects-http-contents)
- [responses > 16384  =>  502 Bad Gateway](#responses--16384----502-bad-gateway)
- [richer logging, use with 'mode tcp' - shows conn numbers, frontend + backend](#option----------tcplog--------------richer-logging-use-with-mode-tcp---shows-conn-numbers-frontend--backend)
- [logs every health check than just changes, this way can see different failur types eg. fail vs conn refused vs timeouts](#option----------log-health-checks---logs-every-health-check-than-just-changes-this-way-can-see-different-failur-types-eg-fail-vs-conn-refused-vs-timeouts)
- [capture Content-length to logs, limiting to 10 chars](#capture-request--header-content-length-10---capture-content-length-to-logs-limiting-to-10-chars)
- [       response](#response)
- [http pipelining uses keep alive - sends multiple reqs before waiting for response, eliminating response latency (eg. image loads)](#http-pipelining-uses-keep-alive---sends-multiple-reqs-before-waiting-for-response-eliminating-response-latency-eg-image-loads)
- [use all backups not just first one](#option---------allbackups-----------use-all-backups-not-just-first-one)
- [#option         checkcache](#option---------checkcache)
- [client tcp keepalives (stops Juniper firewalls from closing idle connections) - socket level only](#option---------clitcpka-------------client-tcp-keepalives-stops-juniper-firewalls-from-closing-idle-connections---socket-level-only)
- [server tcp keepalives](#option---------srvtcpka-------------server-tcp-keepalives)
- [both client + server keepalives](#option---------tcpka----------------both-client--server-keepalives)
- [continuous stats collection for smoother graphs with ong session counts (otherwise only updates stats on session close)](#option---------contstats------------continuous-stats-collection-for-smoother-graphs-with-ong-session-counts-otherwise-only-updates-stats-on-session-close)
- [do not delay ACK to skip one ACK when network packet tracing as it's confusing](#no-option-tcp-smart-connect---------do-not-delay-ack-to-skip-one-ack-when-network-packet-tracing-as-its-confusing)
- [Health Checks](#health-checks)
- [bind *:8443 ssl crt blah.pem](#bind-8443-ssl-crt-blahpem)
- [s1 appears in logs](#s1-appears-in-logs)
- [if port not specified then uses port from frontend that client used to connect to backend server (don't omit backend ports - could change frontend port to avoid conflict eg. Ambari vs Presto 8080)](#if-port-not-specified-then-uses-port-from-frontend-that-client-used-to-connect-to-backend-server-dont-omit-backend-ports---could-change-frontend-port-to-avoid-conflict-eg-ambari-vs-presto-8080)
- [else libc (system resolver - /etc/resolv.conf, /etc/hosts)](#else-libc-system-resolver---etcresolvconf-etchosts)
- [else none (ignore server - do not fail startup) [default: last,libc]](#else-none-ignore-server---do-not-fail-startup-default-lastlibc)
- [#hash-type map-type or consistent - see docs for different algos](#hash-type-map-type-or-consistent---see-docs-for-different-algos)
- [insert   - sets SERVERID cookie if client doesn't already supply one](#insert-----sets-serverid-cookie-if-client-doesnt-already-supply-one)
- [indirect - replaces cookie from server, removes cookie from client request so persistence mechanism is totally transparent to application](#indirect---replaces-cookie-from-server-removes-cookie-from-client-request-so-persistence-mechanism-is-totally-transparent-to-application)
- [nocache  - sets "Cache-Control: nocache" HTTP header to prevent caching SERVERID in proxies as should be individual to user](#nocache----sets-cache-control-nocache-http-header-to-prevent-caching-serverid-in-proxies-as-should-be-individual-to-user)
- [DNS](#dns)
- [UserLists](#userlists)
- [ACLs](#acls)
- [see docs for full ACL usage](#see-docs-for-full-acl-usage)
